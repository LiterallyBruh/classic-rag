...
(async () => {
  ...
  stat(0.5, "27 / 29", "ответов GigaChat содержат выдуманную «цитату»");
  stat(3.62, "2 / 15", "верных адресов главы — там, где адрес спрашивался");
  ...
  s.addNotes("Мы не просто заявляем проблему — мы её измерили: GigaChat через API, без доступа к тексту, на наших 29 вопросах: 27 ответов из 29 содержат выдуманную цитату (ни одна закавыченная «цитата» не нашлась в тексте дословно), адрес главы верен 2 раза из 15, где он спрашивался. Для школьника перед ЕГЭ такой ответ опасен.");
  ...
  let bx = seg(BX, 3.05, 27, BAD, "27 ответов с «цитатой» — все цитаты выдуманы", WHITE);
  bx = seg(bx, 3.05, 2, "C9BFB4", null);
  s.addText("2 ответа вовсе без цитаты", { x: bx - 2.75, y: 3.49, w: 2.75, h: 0.22, fontSize: 9, color: MUT, align: "right", margin: 0 });
  s.addText([
  ...
  bx = seg(BX, 3.85, 8, OK, "8 с дословной цитатой", WHITE);
  const redX = bx;
  bx = seg(bx, 3.85, 1, BAD, null);
  bx = seg(bx, 3.85, 9, "C9BFB4", "9 ответов без цитаты", INK);
  bx = seg(bx, 3.85, 9, "E5DED4", "9 честных отказов", MUT);
  const ambX = bx;
  bx = seg(bx, 3.85, 2, "EF9F27", null);
  s.addText("1 сжатая цитата", { x: redX - 0.55, y: 4.29, w: 1.35, h: 0.22, fontSize: 9, color: BAD, margin: 0 });
  s.addText("2 заблокировала модерация Сбера", { x: ambX - 2.3, y: 4.29, w: 2.78, h: 0.22, fontSize: 9, color: MUT, align: "right", margin: 0 });
  s.addShape(p.shapes.ROUNDED_RECTANGLE, { x: 0.5, y: 4.64, w: 9.0, h: 0.68, rectRadius: 0.09, fill: { color: "F3E9E9" } });
  s.addText([
    { text: "10", options: { color: INK } },
    { text: " ≈ ", options: { color: MUT } },
    { text: "11", options: { color: INK } },
  ], { x: 0.62, y: 4.7, w: 1.75, h: 0.38, align: "center", fontSize: 24, bold: true, fontFace: "Cambria", margin: 0 });
  s.addText("верных по сути · из 29", { x: 0.62, y: 5.08, w: 1.75, h: 0.2, align: "center", fontSize: 9, color: MUT, margin: 0 });
  s.addText([
    { text: "Модель не стала «умнее» — но у baseline даже верные ответы подкреплены выдуманной цитатой, их не отличить от неверных. ", options: {} },
    { text: "Вклад RAG: верное можно проверить, вместо вымысла — отказ.", options: { bold: true, color: BERRY } },
  ], { x: 2.5, y: 4.64, w: 6.85, h: 0.68, fontSize: 11.5, color: INK, align: "left", valign: "middle", margin: 0 });
  s.addNotes("Импакт — измерение, а не опрос: тот же GigaChat, те же 29 вопросов; baseline — модель через API без доступа к тексту, наш вариант — плюс retrieval по роману. Три числа сверху: выдуманные цитаты 27→1, причём единственный случай у нас — сжатая настоящая цитата, и метрика его поймала; верные адреса 2→12 из 15; честные отказы 0→9. Полосы показывают состав всех 29 ответов — без выборочности. Легенда полосы RAG: «ответ без цитаты» — ответ дан, часто с адресом главы, но без закавыченной выдержки из романа (метрика дословности к такому неприменима, поэтому это отдельный сегмент); «заблокировала модерация Сбера» — на 2 вопроса («Посвящение» Фауста и «природа Мефистофеля») GigaChat вместо ответа вернул заглушку «генеративные модели не обладают собственным мнением» — это отказ провайдера, не наш «в тексте ответа нет». И честная рамка внизу, снимающая кажущееся противоречие: «верен по сути» и «цитата выдумана» — свойства одного и того же ответа: baseline может верно назвать героя и тут же «подтвердить» это сочинённой цитатой. По сути верны одинаково (10 и 11 из 29), но все 10 верных ответов baseline обёрнуты в выдуманные цитаты — пользователь не отличит их от 19 неверных, всё звучит одинаково убедительно. RAG не добавляет модели знаний: он делает верное проверяемым, а вместо вымысла отказывается; для школьника перед ЕГЭ ложная цитата опаснее отказа. Если спросят про точность цитат там, где они есть: у baseline 0 из 27 дословных, у нас 8 из 9. Все метрики автоматические, сырые ответы в eval/baseline_raw.json.");
